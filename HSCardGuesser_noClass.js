...
function clueGenerator() {
	switch (card.type) {
		case "spell":
			...
			if (card.Tries - 1 >= spellClues.length) {
				gameOver();
			} else {
				...
				lastGuesses.insertAdjacentHTML(
					"beforeend",
					`<p>${lastGuessesList[lastGuessesList.length - 1]}</p>`
				);
			}
		...
		case "minion":
			...
			if (card.Tries - 1 >= minionClues.length) {
				gameOver();
			} else {
				clues.insertAdjacentHTML(
					"beforeend",
		...
		case "weapon":
			...
			if (card.Tries - 1 >= weaponClues.length) {
				gameOver();
			} else {
				clues.insertAdjacentHTML(
					"beforeend",
		...
		case "hero":
			...
			if (card.Tries - 1 >= heroClues.length) {
				gameOver();
			} else {
				clues.insertAdjacentHTML(
					"beforeend",
...
function gameOver() {
	switch (card.Tries) {
		case 15:
			...
			console.log(
				`You are insistent, aren't you...? The card was ${card.title}.`
			);
